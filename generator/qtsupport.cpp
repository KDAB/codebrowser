...
/**
 * \a obj is an expression to a type of an QObject (or pointer to) that is the
 *sender or the receiver
 * \a method is an expression like SIGNAL(....)  or SLOT(....)
 *
 * This function try to find the matching signal or slot declaration, and
 *register its use.
 */
void QtSupport::handleSignalOrSlot(clang::Expr *obj, clang::Expr *method) {
  ...
  const clang::StringLiteral *methodLiteral =
      clang::dyn_cast<clang::StringLiteral>(method);
  if (!methodLiteral) {
    // try qFlagLocation
    ...
    methodLiteral = clang::dyn_cast<clang::StringLiteral>(
        flagLoc->getArg(0)->IgnoreImpCasts());
    if (!methodLiteral)
      return;
  }
  ...
  while (classIt) {
    ...
    for (auto mi = classIt->method_begin(); mi != classIt->method_end(); ++mi) {
      ...
      if (!d_func && (*mi)->getName() == "d_func" &&
          !getResultType(*mi).isNull())
        d_func = *mi;
    }

    // Look in the first base  (because the QObject need to be the first base
    // class)
  ...
  while (argPos < signature.size() && !candidates.empty()) {
    ...
    llvm::StringRef argument =
        signature.substr(argPos, searchPos - argPos).trim();
      ...
      while (sigIt != argument.end() && parIt != typeString.end()) {
        if (*sigIt == *parIt) {
          ...
          ++parIt;
                   ...
                   llvm::StringRef(sigIt, 9).startswith("nsigned ")) {
          // skip unsigned
          sigIt += 8;
        } else if (*parIt == 'n' &&
                   llvm::StringRef(parIt, 9).startswith("nsigned ")) {
          // skip unsigned
          parIt += 8;
        } else {
          break;
  ...
  candidates.erase(std::remove_if(candidates.begin(), candidates.end(),
                     ...
                     return it->getMinRequiredArguments() > arg &&
                            !(it->getNumParams() == arg + 1 &&
                              it->getParamDecl(arg)->getType().getAsString(
                                  policy) == "QPrivateSignal");
                   }),
                   candidates.end());
  ...
  if (methodLiteral->getNumConcatenated() >= 2) {
    ...
    // Goes two level up in the macro expension:  First level is the #
    // expansion,  Second level is SIGNAL macro
    ...
    auto skip = clang::Lexer::MeasureTokenLength(range.getBegin(), sm,
                                                 annotator.getLangOpts());
...
void QtSupport::visitCallExpr(clang::CallExpr *e) {
  ...
  if (methodDecl->getParent()->getName() == "QObject" &&
      (methodDecl->getName() == "connect" ||
       methodDecl->getName() == "disconnect")) {
    ...
    } else if (clang::CXXMemberCallExpr *me =
                   clang::dyn_cast<clang::CXXMemberCallExpr>(e)) {
      if (e->getNumArgs() >= 3) {
        handleSignalOrSlot(e->getArg(0), e->getArg(1));
  ...
  if (methodDecl->getParent()->getName() == "QTimer" &&
      methodDecl->getName() == "singleShot") {
    if (e->getNumArgs() >= 3) {
      handleSignalOrSlot(e->getArg(1), e->getArg(2));
    }
  }
  if (methodDecl->getParent()->getName() == "QHostInfo" &&
      methodDecl->getName() == "lookupHost") {
    if (e->getNumArgs() >= 3) {
      handleSignalOrSlot(e->getArg(1), e->getArg(2));
    }
  }
  if (methodDecl->getParent()->getName() == "QNetworkAccessCache" &&
      methodDecl->getName() == "requestEntry") {
    if (e->getNumArgs() >= 3) {
      handleSignalOrSlot(e->getArg(1), e->getArg(2));
    }
  }
  if (methodDecl->getParent()->getName() == "QDBusAbstractInterface" &&
      methodDecl->getName() == "callWithCallback") {
    if (e->getNumArgs() == 4) {
      handleSignalOrSlot(e->getArg(2), e->getArg(3));
  ...
  if (methodDecl->getParent()->getName() == "QMenu" &&
      methodDecl->getName() == "addAction") {
  ...
  if (methodDecl->getParent()->getName() == "QToolbar" &&
      methodDecl->getName() == "addAction") {
    if (e->getNumArgs() == 3) {
      handleSignalOrSlot(e->getArg(1), e->getArg(2));
