 ...
 *perhaps)
 *  and want to store a relative path so that the group of files can be moved to
 *a different
 *  directory without breaking the paths. An IDE would be a simple example, so
 *that the
...
std::string naive_uncomplete(llvm::StringRef base, llvm::StringRef path) {
  ...
  if (sys::path::has_root_path(path)) {
    if (sys::path::root_path(path) != sys::path::root_path(base)) {
      ...
      return naive_uncomplete(sys::path::relative_path(base),
                              sys::path::relative_path(path));
    }
  } else {
    if (sys::path::has_root_path(base)) {
      std::cerr
          << "naive_uncomplete(" << base.str() << "," << path.str()
