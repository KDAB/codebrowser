...
struct BrowserASTVisitor : clang::RecursiveASTVisitor<BrowserASTVisitor> {
  ...
  bool VisitTypedefNameDecl(clang::TypedefNameDecl *d) {
    annotator.registerReference(d, d->getLocation(), Annotator::Typedef,
                                Annotator::Declaration,
      ...
      if (clang::CXXRecordDecl *cxx = llvm::dyn_cast<clang::CXXRecordDecl>(d)) {
        for (auto it = cxx->bases_begin(); it != cxx->bases_end(); ++it) {
          if (!it->getType()->getAsCXXRecordDecl()) {
            //                        std::cerr << " INHERITING but not from a
            //                        CXXRecrod " << std::endl;
            //                        it->getType().dump();
            // probably template type...  FIXME
            continue;
          }
          annotator.registerOverride(d, it->getType()->getAsCXXRecordDecl(),
                                     d->getLocation());
        }
      }
    }
  ...
  bool VisitNamespaceDecl(clang::NamespaceDecl *d) {
    annotator.registerReference(d, d->getLocation(), Annotator::Namespace,
                                Annotator::Declaration);
    return true;
  }
  ...
  bool VisitFunctionDecl(clang::FunctionDecl *d) {
    ...
    std::string typeText;
    {
      ...
      if (clang::CXXMethodDecl *cxx = llvm::dyn_cast<clang::CXXMethodDecl>(d)) {
        ...
        if (cxx->isThisDeclarationADefinition()) {
          for (auto it = cxx->begin_overridden_methods();
               it != cxx->end_overridden_methods(); ++it) {
            const clang::CXXMethodDecl *ovr = (*it)->getCanonicalDecl();
            annotator.registerOverride(d,
                                       const_cast<clang::CXXMethodDecl *>(ovr),
                                       d->getNameInfo().getBeginLoc());
          }
        }
      }
      typeTextStream << annotator.getTypeRef(getResultType(d)) << " "
                     << d->getQualifiedNameAsString() << "(";
      for (uint i = 0; i < d->getNumParams(); i++) {
        if (i != 0)
          typeTextStream << ", ";
        clang::ParmVarDecl *PVD = d->getParamDecl(i);
        typeTextStream << annotator.getTypeRef(PVD->getType()) << " "
                       << PVD->getName();
        if (PVD->hasDefaultArg() && !PVD->hasUninstantiatedDefaultArg()) {
          typeTextStream << " = ";
          PVD->getDefaultArg()->printPretty(typeTextStream, 0,
    ...
    annotator.registerReference(
        d, d->getNameInfo().getSourceRange(), Annotator::Decl,
        d->isThisDeclarationADefinition() ? Annotator::Definition
                                          : Annotator::Declaration,
        typeText);
    return true;
  }
  ...
  bool VisitMemberExpr(clang::MemberExpr *e) {
    annotator.registerUse(
        e->getMemberDecl(), e->getMemberNameInfo().getSourceRange(),
  ...
  bool VisitDeclRefExpr(clang::DeclRefExpr *e) {
    ...
    annotator.registerUse(decl, e->getNameInfo().getSourceRange(),
                          llvm::isa<clang::EnumConstantDecl>(decl)
                              ? Annotator::EnumDecl
                              : isMember(decl) ? Annotator::Member
                                               : Annotator::Ref,
                          currentContext);
    return true;
  }
  ...
  bool VisitTagTypeLoc(clang::TagTypeLoc TL) {
    clang::SourceRange range = TL.getSourceRange();
    annotator.registerUse(TL.getDecl(), range.getBegin(), Annotator::Type,
                          currentContext);
    return true;
  }

  bool
  VisitTemplateSpecializationTypeLoc(clang::TemplateSpecializationTypeLoc TL) {
    ...
    if (decl) {
      ...
      std::cerr << "VisitTemplateSpecializationTypeLoc "
  ...
  bool TraverseNestedNameSpecifierLoc(clang::NestedNameSpecifierLoc NNS) {
    ...
    case clang::NestedNameSpecifier::Namespace:
      if (NNS.getNestedNameSpecifier()
              ->getAsNamespace()
              ->isAnonymousNamespace())
        break;
      annotator.registerReference(
          NNS.getNestedNameSpecifier()->getAsNamespace(), NNS.getSourceRange(),
          Annotator::Namespace);
      return true; // skip prefixes
    case clang::NestedNameSpecifier::NamespaceAlias:
      annotator.registerReference(NNS.getNestedNameSpecifier()
                                      ->getAsNamespaceAlias()
                                      ->getAliasedNamespace(),
  ...
  bool TraverseConstructorInitializer(clang::CXXCtorInitializer *Init) {
    if (Init->isAnyMemberInitializer() && Init->isWritten()) {
      annotator.registerUse(Init->getAnyMember(), Init->getMemberLocation(),
                            Init->isMemberInitializer() ? Annotator::Member
                                                        : Annotator::Ref,
                            currentContext);
    }
...
private:
  bool isMember(clang::NamedDecl *d) {
    ...
    clang::CXXRecordDecl *ctx =
        llvm::dyn_cast<clang::CXXRecordDecl>(currentContext->getDeclContext());
    if (!ctx)
      return false;
    ...
    clang::CXXRecordDecl *rec =
        llvm::dyn_cast<clang::CXXRecordDecl>(d->getDeclContext());
